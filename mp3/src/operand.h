...
public:
	...
	bool is_Obj()
	{
		return id == OBJ || id == OBJ_PTR || id == OBJ_PPTR;
	}
	bool is_ptr()
	{
...
public:
	...
	void set_local(bool isLocal) { local = isLocal; }
	bool is_self_ret() { return self_type_ret; }
	void set_self_ret(bool isSelf) { self_type_ret = isSelf; }
};
...
public:
	casted_value(op_type t, string val, op_type precast_t) : const_value(t, "bitcast (" + precast_t.get_name() + " " + val + " to " + t.get_name() + ")", true),
