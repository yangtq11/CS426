...
public:
	...
	ostream *ct_stream;
	op_type main_return_type = op_type("Object*");
...
public:
	...
	vector<global_value> string_global_value;
};
...
public:
	...
	op_type get_type(string type_decl);
	void setup_buildin_vtable();
	void setup_inherited_class();
	int get_attribute_index(string attri_name);

	vector<op_type> attri_list;
	vector<string> attri_name_list;
	vector<operand> formal_ops;

	vector<op_func_type> vtable_funcs;

	vector<op_type> vtable_types;
	vector<op_func_type> vtable_func_types;
	vector<const_value> vtable_values;
	vector<string> vtable_names;

	vector<op_type> extra_v_types;
	vector<const_value> extra_v_values;
	vector<string> extra_v_return_types;
	vector<op_func_type> extra_v_func_types;
	vector<op_func_type> extra_v_ori_func_types;

	vector<op_type> inherit_io_types;
	vector<op_func_type> inherit_io_func_types;
	vector<op_func_type> inherit_io_ori_func_types;
	vector<const_value> inherit_io_values;
	vector<op_type> inherit_io_self_types;
...
public:
	...
	// ADD CODE HERE
	int get_symbol_index(string name);
	bool method;
	operand real_type;
	vector<Symbol> symbols;
	vector<operand> func_arguments;
};
