...
{ 
	...
	operand Variable = *env->lookup(name);
	//get the assigned value
	operand Value = expr->code(env);
	vp.store(Value, Variable);
	return Value;
}
...
{ 
	...
	if(emptyValue){
		string defalutValue;
		if(idType.get_id()==INT32){
			defalutValue = "0";
		}else if(idType.get_id()==INT1){
			defalutValue = "false";
		}
		vp.store(const_value(idType, defalutValue, true), idPtr);
	}else{
		vp.store(idValue, idPtr);
	}
...
operand object_class::code(CgenEnvironment *env) 
{
	...
	//find the variable in the local scope
	operand Variable = *env->lookup(name);
	return vp.load(Variable.get_type().get_deref_type(),Variable);
}
